...
Once that command buffer has been submitted the compute shader will spin through every particle in the buffer and set it's poition. So now we should have a buffer full of particle positions laid out in a big grid! To verify that, let's put together a render pipeline that can draw the particles for us in a simplistic fashion.
